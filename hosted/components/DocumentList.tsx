import type { SectionDoc } from "@/lib/render-types";

export function DocumentList({ docs }: { docs: SectionDoc[] }) {
  if (!docs || docs.length === 0) return null;
  return (
    <div className="mt-10 not-prose">
      <h3 className="text-sm font-semibold uppercase tracking-wide text-zinc-400 mb-3">Documents</h3>
      <ul className="divide-y divide-zinc-200 dark:divide-zinc-800 border-y border-zinc-200 dark:border-zinc-800">
        {docs.map((d) => (
          <li key={d.docId}>
            <a
              href={`/room/docs/${d.docId}`}
